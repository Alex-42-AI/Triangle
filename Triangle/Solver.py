...
def side_angle_side1(side, side1, angle, angle1, angle2, h, h1, h2):
    ...
    if angle1 or angle2:
        if angle1:
            ...
            if abs(angle1 - a_angle1) <= abs(angle1 - b_angle1):
                angle1 = a_angle1
            ...
            if abs(angle2 - a_angle2) <= abs(angle2 - b_angle2):
                angle2 = a_angle2
...
def get_coordinates(res_triangle: list[float]):
    ...
    coordinates = [(0, 0), B_coords, C_coords, ((B_coords[0] + C_coords[0]) / 3, C_coords[1] / 3)]
...
def solve(a=0, b=0, c=0, A=0, B=0, C=0, h_a=0, h_b=0, h_c=0):
